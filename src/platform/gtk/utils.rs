...
pub fn gtk_key_to_script_key(key: gdk::key) -> Result<Key, ()> {
    Err(())
    /*match key {
...
pub fn is_printable(key_code: gdk::key) -> bool {
    false
...
pub fn servo_cursor_to_gtk_cursor(servo_cursor: ServoCursor) -> gdk::Cursor {
    let cursor_name = match servo_cursor {
        ServoCursor::None => "none",
        ServoCursor::Default => "default",
        ServoCursor::Pointer => "pointer",
        ServoCursor::ContextMenu => "context-menu",
        ServoCursor::Help => "help",
        ServoCursor::Progress => "progress",
        ServoCursor::Wait => "wait",
        ServoCursor::Cell => "cell",
        ServoCursor::Crosshair => "crosshair",
        ServoCursor::Text => "text",
        ServoCursor::VerticalText => "vertical-text",
        ServoCursor::Alias => "alias",
        ServoCursor::Copy => "copy",
        ServoCursor::Move => "move",
        ServoCursor::NoDrop => "no-drop",
        ServoCursor::NotAllowed => "not-allowed",
        ServoCursor::Grab => "grab",
        ServoCursor::Grabbing => "grabbing",
        ServoCursor::EResize => "e-resize",
        ServoCursor::NResize => "n-resize",
        ServoCursor::NeResize => "ne-resize",
        ServoCursor::NwResize => "nw-resize",
        ServoCursor::SResize => "s-resize",
        ServoCursor::SeResize => "se-resize",
        ServoCursor::SwResize => "sw-resize",
        ServoCursor::WResize => "w-resize",
        ServoCursor::EwResize => "ew-resize",
        ServoCursor::NsResize => "ns-resize",
        ServoCursor::NeswResize => "nesw-resize",
        ServoCursor::NwseResize => "nwse-resize",
        ServoCursor::ColResize => "col-resize",
        ServoCursor::RowResize => "row-resize",
        ServoCursor::AllScroll => "all-scroll",
        ServoCursor::ZoomIn => "zoom-in",
        ServoCursor::ZoomOut => "zoom-out",
    };
    let display = Display::get_default().unwrap();
    gdk::Cursor::new_from_name(&display, cursor_name)
}

// Some shortcuts use Cmd on Mac and Control on other systems.
pub fn cmd_or_ctrl(modifiers: gdk::ModifierType) -> bool {
    if cfg!(target_os = "macos") {
        modifiers.contains(gdk::SUPER_MASK) // TODO: check
    } else {
        modifiers.contains(gdk::CONTROL_MASK)
    }
}
